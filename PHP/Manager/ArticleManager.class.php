...
//article additions section
    function addArticle($name_article, $description_article, $quantity_article, $price_article, $image){
        $query = $this->db->prepare("INSERT INTO article (name_article, description_article, quantity_article, price_article, image) VALUES (:name_article, :description_article, :quantity_article, :price_article, :image)");
        $query->bindValue(':name_article', $name_article);
...
//article feature section 
    ...
    function totalCart (){
        $total = 0;
        $quantity_article = count($_SESSION['panier']);
        for($i = 0; $i < $quantity_article; $i++){
            $total += $_SESSION['panier'][$i][1];
